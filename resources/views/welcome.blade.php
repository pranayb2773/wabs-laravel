<x-layouts.guest title="WABS.AI | Wealth Labs - AI-Powered Trading Journal">
    {{-- Hero Section --}}
    <section class="min-h-screen flex flex-col justify-center items-center text-center px-8 pt-32 pb-16 relative overflow-hidden">
        {{-- Animated gradient orbs --}}
        <div class="absolute w-[900px] h-[900px] -top-[300px] -right-[200px] bg-[radial-gradient(circle,rgba(168,85,247,0.2)_0%,rgba(124,58,237,0.1)_40%,transparent_70%)] animate-float"></div>
        <div class="absolute w-[700px] h-[700px] -bottom-[150px] -left-[150px] bg-[radial-gradient(circle,rgba(217,70,239,0.15)_0%,rgba(168,85,247,0.05)_50%,transparent_70%)] animate-float-reverse"></div>
        <div class="absolute w-[500px] h-[500px] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[radial-gradient(circle,rgba(192,132,252,0.1)_0%,transparent_60%)] animate-pulse-glow"></div>

        <div class="relative z-10 max-w-4xl">
            <div class="inline-flex items-center gap-2 px-5 py-2 bg-brand-primary/15 border border-brand-primary/30 rounded-full font-mono text-xs text-brand-tertiary mb-8 animate-fade-in-up">
                <span class="w-1.5 h-1.5 bg-brand-primary rounded-full animate-pulse shadow-lg shadow-brand-primary"></span>
                <span>Release v.01 — Now Live</span>
            </div>

            <h1 class="font-display text-5xl sm:text-6xl lg:text-7xl font-normal leading-tight mb-6 tracking-tight animate-fade-in-up animation-delay-100">
                Smarter Moves,<br />
                <em class="italic bg-gradient-to-br from-brand-primary via-brand-secondary to-brand-glow bg-clip-text text-transparent">Stronger Results</em>
            </h1>

            <p class="text-xl text-brand-text-secondary max-w-xl mx-auto mb-12 leading-relaxed animate-fade-in-up animation-delay-200">
                Powered by AI, our trading journal analyzes every move, reveals key insights, and drives better trading outcomes.
            </p>

            <div class="flex gap-4 justify-center flex-wrap animate-fade-in-up animation-delay-300">
                <a href="{{ route('login') }}" class="px-8 py-4 rounded-full font-body font-semibold text-base bg-gradient-to-br from-brand-primary via-brand-secondary to-brand-glow text-white shadow-lg shadow-brand-primary/40 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-brand-primary/50">
                    Start Trading Smarter
                </a>
                <a href="#" class="px-8 py-4 rounded-full font-body font-semibold text-base bg-transparent text-brand-text border border-brand-primary/30 transition-all hover:bg-brand-primary/10 hover:border-brand-primary/50">
                    Watch Demo
                </a>
            </div>
        </div>
    ...
    <section class="px-8 py-16 border-t border-b border-brand-primary/10 overflow-hidden bg-brand-card/50">
        ...
        <div class="flex gap-16 animate-marquee">
            @foreach(['Robinhood', 'Webull', 'E*TRADE', 'Fidelity', 'Interactive Brokers', 'Charles Schwab', 'ThinkorSwim', 'Tastytrade', 'Topstep', 'Apex', 'Tradeify', 'Robinhood', 'Webull', 'E*TRADE', 'Fidelity', 'Interactive Brokers', 'Charles Schwab', 'ThinkorSwim', 'Tastytrade', 'Topstep', 'Apex', 'Tradeify'] as $broker)
                <span class="font-body font-semibold text-lg text-brand-text-muted whitespace-nowrap opacity-50 transition-all hover:opacity-100 hover:text-brand-tertiary">{{ $broker }}</span>
            @endforeach
        </div>
    </section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            ...
            @foreach($features as $feature)
                <div class="group bg-gradient-to-br from-brand-primary/10 to-brand-secondary/5 border border-brand-primary/15 rounded-3xl p-10 relative overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:border-brand-primary/30 hover:shadow-2xl hover:shadow-brand-primary/15 hover:from-brand-primary/15 hover:to-brand-secondary/8">
                    ...
                    <div class="w-14 h-14 bg-gradient-to-br from-brand-primary/20 to-brand-secondary/10 border border-brand-primary/20 rounded-2xl flex items-center justify-center mb-6 text-2xl">
                        {{ $feature['icon'] }}
                    </div>
    ...
    {{-- Showcase Section --}}
    <section class="px-8 lg:px-16 py-32 bg-gradient-to-b from-brand-card to-brand-deep relative">
        <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(168,85,247,0.05)_0%,transparent_70%)]"></div>
        ...
        <div class="grid lg:grid-cols-2 gap-24 items-center max-w-7xl mx-auto relative z-10">
            <div>
                <h2 class="font-display text-3xl lg:text-4xl font-normal leading-tight mb-6">
                    Deep Trade Analytics<br />Tailored to You
                </h2>
                <p class="text-brand-text-secondary text-lg leading-relaxed mb-8">
                    See every detail of your execution—from entries to R-multiples—in one clean dashboard. Wealth Labs breaks down your performance so you can optimize your strategy with confidence.
                </p>
                <a href="{{ route('features') }}" class="inline-block px-8 py-4 rounded-full font-body font-semibold text-base bg-gradient-to-br from-brand-primary via-brand-secondary to-brand-glow text-white shadow-lg shadow-brand-primary/40 transition-all hover:-translate-y-0.5 hover:shadow-xl hover:shadow-brand-primary/50">
                    Explore Analytics
                </a>
            </div>
    ...
    <section class="px-8 lg:px-16 py-24 border-t border-brand-primary/10">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto text-center">
            ...
            @foreach($statsDisplay as $stat)
                <div class="p-8">
                    <div class="font-display text-5xl lg:text-6xl font-normal bg-gradient-to-br from-brand-primary via-brand-secondary to-brand-glow bg-clip-text text-transparent leading-none mb-2">
                        {{ $stat['number'] }}
                    </div>
    ...
    <section id="tools" class="px-8 lg:px-16 py-32 relative">
        <div class="max-w-7xl mx-auto">
            <div class="text-center mb-20">
                ...
                <h2 class="font-display text-4xl lg:text-5xl font-normal tracking-tight">Powerful Trading Intelligence</h2>
            </div>
            ...
            @foreach($tools as $index => $tool)
                <div class="grid lg:grid-cols-2 gap-16 items-center mb-32 last:mb-0 {{ $index % 2 === 1 ? 'lg:[direction:rtl]' : '' }}">
                    ...
                    <div class="bg-gradient-to-br from-brand-primary/10 to-brand-secondary/5 border border-brand-primary/15 rounded-2xl p-8 min-h-[300px] flex items-center justify-center relative overflow-hidden {{ $index % 2 === 1 ? 'lg:[direction:ltr]' : '' }}">
                        <div class="absolute inset-0 bg-gradient-to-br from-brand-primary/8 to-transparent"></div>
                        <div class="w-full h-[200px] flex items-end gap-2 p-4 relative z-10">
                            @for($i = 0; $i < 10; $i++)
                                <div class="flex-1 bg-gradient-to-t from-brand-primary to-brand-glow rounded-t opacity-70 transition-opacity hover:opacity-100 shadow-lg shadow-brand-primary/30" style="height: {{ rand(35, 95) }}%"></div>
                            @endfor
                        </div>
                    </div>
                </div>
    ...
    <section class="px-8 lg:px-16 py-40 text-center relative overflow-hidden">
        ...
        <div class="relative z-10 max-w-2xl mx-auto">
            <h2 class="font-display text-4xl lg:text-5xl font-normal mb-6">
                Smart Journaling<br />Starts <em class="italic bg-gradient-to-br from-brand-primary via-brand-secondary to-brand-glow bg-clip-text text-transparent">Now</em>
            </h2>
            <p class="text-brand-text-secondary text-lg mb-12">
                Join thousands of traders who are already using AI to decode their trading patterns and build sustainable profitability.
            </p>
